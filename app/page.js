...
export default function Home() {
  ...
  const [form, setForm] = useState({ company: '', position: '', date: '', status: 'Applied', resume: '', coverLetter: '', notes: '' })
...
[PASTE THE JOB DESCRIPTION HERE]`
  ...
  useEffect(() => { const saved = localStorage.getItem('jobs'); if (saved) setJobs(JSON.parse(saved)); const s = localStorage.getItem('mystate'); if (s) setState(s) }, [])
  useEffect(() => { localStorage.setItem('jobs', JSON.stringify(jobs)) }, [jobs])
  useEffect(() => { localStorage.setItem('mystate', state) }, [state])
  const add = () => { if (form.company && form.position) { setJobs([...jobs, { ...form, id: Date.now() }]); setForm({ company: '', position: '', date: '', status: 'Applied', resume: '', coverLetter: '', notes: '' }) } }
  ...
  const formatText = (text) => text.split('\n').map((line, i) => {
    const isHeader = line === line.toUpperCase() && line.length > 2 && line.trim().length > 0
    const isBullet = line.trim().startsWith('•') || line.trim().startsWith('-') || line.trim().startsWith('*')
    if (isHeader) return <div key={i} style={{fontWeight:700,fontSize:16,marginTop:16,marginBottom:8,color:'#1f2937',borderBottom:'1px solid #e5e7eb',paddingBottom:4}}>{line}</div>
    if (isBullet) return <div key={i} style={{paddingLeft:20,marginBottom:4,color:'#374151'}}>{line}</div>
    if (line.trim() === '') return <div key={i} style={{height:8}}/>
    return <div key={i} style={{marginBottom:4,color:'#374151'}}>{line}</div>
  })
    ...
    <div style={{maxWidth:1200,margin:'0 auto',padding:20,paddingBottom:80}}>
      ...
      <div style={{background:'#f0f9ff',border:'1px solid #bae6fd',borderRadius:8,padding:16,marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
          <div style={{background:'#fff',border:'1px solid #bae6fd',borderRadius:6,padding:12}}>
            <div style={{fontWeight:600,color:'#0369a1',marginBottom:8}}>📄 Resume</div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>copyWithState(resumePrompt,'resume-copy')} style={{flex:1,padding:10,background:copied==='resume-copy'?'#22c55e':'#0ea5e9',color:'#fff',border:'none',borderRadius:4,cursor:'pointer',fontSize:13}}>{copied==='resume-copy'?'✓ Copied!':'Copy Prompt + State'}</button>
              <button onClick={()=>setShowPrompt('resume')} style={{padding:10,background:'#fff',color:'#0369a1',border:'1px solid #0369a1',borderRadius:4,cursor:'pointer',fontSize:13}}>View</button>
            </div>
          </div>
          <div style={{background:'#fff',border:'1px solid #bae6fd',borderRadius:6,padding:12}}>
            <div style={{fontWeight:600,color:'#0369a1',marginBottom:8}}>✉️ Cover Letter</div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>copyWithState(coverLetterPrompt,'cover-copy')} style={{flex:1,padding:10,background:copied==='cover-copy'?'#22c55e':'#0ea5e9',color:'#fff',border:'none',borderRadius:4,cursor:'pointer',fontSize:13}}>{copied==='cover-copy'?'✓ Copied!':'Copy Prompt + State'}</button>
              <button onClick={()=>setShowPrompt('cover')} style={{padding:10,background:'#fff',color:'#0369a1',border:'1px solid #0369a1',borderRadius:4,cursor:'pointer',fontSize:13}}>View</button>
            </div>
          </div>
        </div>
        <button onClick={()=>setShowState(true)} style={{width:'100%',padding:10,background:'#fff',color:'#0369a1',border:'1px solid #0369a1',borderRadius:4,cursor:'pointer',fontSize:13,marginBottom:12}}>{state ? '⚙️ View/Edit State Document' : '⚙️ Add State Document (Required First)'}</button>
        <div style={{background:'#fff',border:'1px solid #bae6fd',borderRadius:4,padding:12,fontSize:14,color:'#0c4a6e'}}>
          <strong>Steps:</strong>
          <ol style={{margin:'8px 0 0',paddingLeft:20,lineHeight:1.8}}>
            ...
            <li>Paste into AI and replace <code style={{background:'#e0f2fe',padding:'2px 6px',borderRadius:3}}>[PASTE THE JOB DESCRIPTION HERE]</code></li>
      ...
      <textarea placeholder="Resume (paste text)" value={form.resume} onChange={e=>setForm({...form,resume:e.target.value})} style={{width:'100%',height:100,padding:10,border:'1px solid #ccc',borderRadius:4,marginBottom:10,boxSizing:'border-box',fontFamily:'inherit'}}/>
      <textarea placeholder="Cover Letter (paste text)" value={form.coverLetter} onChange={e=>setForm({...form,coverLetter:e.target.value})} style={{width:'100%',height:100,padding:10,border:'1px solid #ccc',borderRadius:4,marginBottom:10,boxSizing:'border-box',fontFamily:'inherit'}}/>
      <textarea placeholder="Notes" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} style={{width:'100%',height:60,padding:10,border:'1px solid #ccc',borderRadius:4,marginBottom:10,boxSizing:'border-box',fontFamily:'inherit'}}/>
      ...
      <table style={{width:'100%',borderCollapse:'collapse'}}>
        <thead><tr style={{background:'#f3f4f6'}}>{['Company','Position','Date','Status','Actions'].map(h=><th key={h} style={{padding:12,textAlign:'left',borderBottom:'2px solid #e5e7eb'}}>{h}</th>)}</tr></thead>
        <tbody>
          {jobs.map(j=>(
            <tr key={j.id} style={{borderBottom:'1px solid #e5e7eb'}}>
              ...
              <td style={{padding:12}}>
                <button onClick={()=>{setView(j);setTab('resume')}} style={{marginRight:8,padding:'4px 8px',background:'#6b7280',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>View</button>
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setShowPrompt(null)}>
          <div style={{background:'#fff',borderRadius:8,maxWidth:800,width:'100%',maxHeight:'90vh',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:20,borderBottom:'1px solid #e5e7eb',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <h2 style={{margin:0}}>{showPrompt==='resume'?'Resume':'Cover Letter'} Prompt</h2>
              <button onClick={()=>copy(showPrompt==='resume'?resumePrompt:coverLetterPrompt,'prompt-view')} style={{padding:'8px 16px',background:copied==='prompt-view'?'#22c55e':'#0ea5e9',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>{copied==='prompt-view'?'✓ Copied!':'Copy'}</button>
            </div>
            <div style={{padding:20,overflow:'auto',flex:1}}>
              <pre style={{margin:0,whiteSpace:'pre-wrap',fontFamily:'monospace',fontSize:13,lineHeight:1.6,background:'#f9fafb',padding:16,borderRadius:4}}>{showPrompt==='resume'?resumePrompt:coverLetterPrompt}</pre>
            </div>
            <div style={{padding:15,borderTop:'1px solid #e5e7eb',textAlign:'right'}}>
              <button onClick={()=>setShowPrompt(null)} style={{padding:'10px 20px',background:'#3b82f6',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>Close</button>
            </div>
          </div>
        </div>
      )}
      {showState && (
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>{setShowState(false);setEditState(false)}}>
          <div style={{background:'#fff',borderRadius:8,maxWidth:800,width:'100%',maxHeight:'90vh',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:20,borderBottom:'1px solid #e5e7eb',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <h2 style={{margin:0}}>My State Document</h2>
              <button onClick={()=>setEditState(!editState)} style={{padding:'8px 16px',background:'#6b7280',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>{editState ? 'Done' : 'Edit'}</button>
            </div>
            <div style={{padding:20,overflow:'auto',flex:1}}>
              {editState ? (
                <textarea value={state} onChange={e=>setState(e.target.value)} placeholder="Paste your state document here (all your experience, skills, projects, education, contact info, etc)..." style={{width:'100%',height:'100%',minHeight:400,padding:12,border:'1px solid #ccc',borderRadius:4,boxSizing:'border-box',fontFamily:'monospace',fontSize:13,lineHeight:1.5}}/>
              ) : (
                state ? <pre style={{margin:0,whiteSpace:'pre-wrap',fontFamily:'monospace',fontSize:13,lineHeight:1.6}}>{state}</pre> : <p style={{color:'#999'}}>No state document yet. Click Edit to add one.</p>
              )}
            </div>
            <div style={{padding:15,borderTop:'1px solid #e5e7eb',textAlign:'right'}}>
              <button onClick={()=>{setShowState(false);setEditState(false)}} style={{padding:'10px 20px',background:'#3b82f6',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>Close</button>
            </div>
          </div>
        </div>
      )}
      {view && (
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setView(null)}>
          <div style={{background:'#fff',borderRadius:8,maxWidth:800,width:'100%',maxHeight:'90vh',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:20,borderBottom:'1px solid #e5e7eb'}}>
              <h2 style={{margin:0}}>{view.company} - {view.position}</h2>
              <p style={{margin:'8px 0 0',color:'#666'}}>{view.date} • <span style={{padding:'2px 8px',borderRadius:4,background:statusColor[view.status],color:'#fff',fontSize:12}}>{view.status}</span></p>
            </div>
            <div style={{display:'flex',borderBottom:'1px solid #e5e7eb'}}>
              {['resume','coverLetter','notes'].map(t=>(
                <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:12,border:'none',background:tab===t?'#f3f4f6':'#fff',cursor:'pointer',fontWeight:tab===t?600:400,borderBottom:tab===t?'2px solid #3b82f6':'2px solid transparent'}}>
                  {t==='coverLetter'?'Cover Letter':t.charAt(0).toUpperCase()+t.slice(1)}
                </button>
              ))}
            </div>
            <div style={{padding:20,overflow:'auto',flex:1,lineHeight:1.6}}>
              {view[tab] ? formatText(view[tab]) : <p style={{color:'#999',fontStyle:'italic'}}>No {tab==='coverLetter'?'cover letter':tab} provided</p>}
            </div>
            <div style={{padding:15,borderTop:'1px solid #e5e7eb',textAlign:'right'}}>
              <button onClick={()=>setView(null)} style={{padding:'10px 20px',background:'#3b82f6',color:'#fff',border:'none',borderRadius:4,cursor:'pointer'}}>Close</button>
            </div>
          </div>
        </div>
      )}
