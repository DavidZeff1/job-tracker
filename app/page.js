...
export default function Home() {
  ...
  return (
    <div style={{maxWidth:1200,margin:'0 auto',padding:20}}>
      ...
      <textarea placeholder="Resume (paste text)" value={form.resume} onChange={e=>setForm({...form,resume:e.target.value})} style={{width:'100%',height:80,padding:10,border:'1px solid #ccc',borderRadius:4,marginBottom:10,boxSizing:'border-box'}}/>
      <textarea placeholder="Cover Letter (paste text)" value={form.coverLetter} onChange={e=>setForm({...form,coverLetter:e.target.value})} style={{width:'100%',height:80,padding:10,border:'1px solid #ccc',borderRadius:4,marginBottom:10,boxSizing:'border-box'}}/>
      ...
      {view && (
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.5)',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setView(null)}>
          <div style={{background:'#fff',padding:30,borderRadius:8,maxWidth:800,maxHeight:'80vh',overflow:'auto',width:'90%'}} onClick={e=>e.stopPropagation()}>
            ...
            {view.resume && <><h3>Resume</h3><pre style={{background:'#f3f4f6',padding:15,borderRadius:4,whiteSpace:'pre-wrap',overflow:'auto'}}>{view.resume}</pre></>}
            {view.coverLetter && <><h3>Cover Letter</h3><pre style={{background:'#f3f4f6',padding:15,borderRadius:4,whiteSpace:'pre-wrap',overflow:'auto'}}>{view.coverLetter}</pre></>}
